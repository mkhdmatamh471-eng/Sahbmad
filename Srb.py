...
def manual_fallback_check(clean_text):
    # 1. كلمات تدل على "نية الطلب" (Intent)
    order_triggers = [
        "ابي", "ابغي", "أبغا", "محتاج", "مطلوب", "نبي", "مين", 
        "في احد", "في أحد", "بكم", "يوديني", "يوصلني", "توصيله", "توصيلة"
    ]
    
    # 2. وجهات ومعالم المدينة المنورة (Context)
    madinah_keywords = [
        "سواق", "كابتن", "مشوار", "توصيل", "المدينة", "المدينه", 
        "الحرم", "طيبة", "المركزية", "قباء", "العزيزية", "شوران", 
        "المطار", "القطار", "الراشد", "النور مول", "العالية"
    ]
    
    # 3. فحص "الطلب الصريح" (مثلاً: أبغى مشوار، مطلوب سواق)
    # قللنا القيود هنا: أي كلمة طلب مع أي كلمة سياق
    has_order = any(w in clean_text for w in order_triggers)
    has_keyword = any(w in clean_text for w in madinah_keywords)
    
    # 4. فحص "المسار" (من وإلى) - هذا أقوى مؤشر على وجود عميل
    # يدعم: "من العزيزية للحرم"، "من المطار الى شقة"
    has_route = "من" in clean_text and any(x in clean_text for x in [" الى", " إلى", " لـ", " للحرم", " للمطار", " لحي"])
    
    # 5. فحص "السؤال عن السعر" (بكم من.. ، بكم المشوار)
    is_asking_price = "بكم" in clean_text and any(x in clean_text for x in ["مشوار", "توصيل", "يوديني"])

    # النتيجة: إذا تحقق أي شرط من الثلاثة، اسحب الطلب فوراً
